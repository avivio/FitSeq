...
DEFAULT_FINAL_RESULT_FILE_NAME = '/final_result'
DEFAULT_FINAL_SUMMARY_FILE_NAME = '/final_summary'


DEFAULT_MATCH_COUNT_FILE_NAME = '/all_match_counts.txt'
DEFAULT_FINAL_MATCH_COUNT_FILE_NAME = '/final_match_count'
...
def collect_all_results(final_result_directory,all_result_file_name = DEFAULT_RESULT_FILE_NAME,
    ...
    dir = os.path.basename(os.path.normpath(final_result_directory))
    date_time = dir.split('_')[2]

    max_mismatch =int(dir.split('_')[4])
    ...
    for mismatch in range(max_mismatch+1) + ['all']:
        file_suffix =  '_' + str(mismatch) + '_mismatches.csv'
        #open final result matrix as a csv location in the directory specified by the user
        final_result_csv_location = final_result_directory + final_result_file_name + '_' + date_time +  file_suffix
        ...
        first_result_csv = csv.reader(open(result_files_list[0].strip()+file_suffix,'rb'))
        ...
        for result_file in result_files_list[1:]:
            result_csvs.append(csv.reader(open(result_file.strip()+file_suffix,'rb')))
        ...
        final_match_count_csv_location = final_result_directory + final_match_count_file_name +'_' + date_time + file_suffix
        final_match_count_csv = csv.writer(open(final_match_count_csv_location,'wb'))

        #open first csv
        first_match_count_csv = csv.reader(open(match_count_files_list[0].strip()+file_suffix,'rb'))
        #create csv list
        match_count_csvs = []
        for match_count_file in match_count_files_list[1:]:
            match_count_csvs.append(csv.reader(open(match_count_file.strip()+file_suffix,'rb')))
    ...
    #create final summary file
    final_summary_csv_location = final_result_directory + final_summary_file_name +  '_' + date_time + '.csv'
